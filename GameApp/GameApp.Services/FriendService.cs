...
namespace GameApp.Services
{
    public class FriendService : IFriendService
    {
        ...
        public async Task<bool> ChangeStatus(string userId, string username, string status, string only)
        {
        ...
        {
            return await friends.All().Where(f => f.MainUserId == userId && f.Status=="Friend" || f.FriendUserId == userId && f.Status == "Friend").Select(f => f.MainUserId == userId ? f.FriendUser.UserName : f.MainUser.UserName).ToListAsync();
        }

        public async Task<IEnumerable<string>> GetRequests(string userId)
        {
            return await friends.All().Where(f => f.FriendUserId == userId && f.Status == "Request").Select(f => f.MainUser.UserName).ToListAsync();
        }
